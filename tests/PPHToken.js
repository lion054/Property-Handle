...
contract('PPHToken', function(accounts) {
  var tokenInstance;
  ...
  it('initializes the contract with the correct values', function() {
    return PPHToken.deployed().then(function(instance) {
      ...
      assert.equal(name, 'PPH Token', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'PPH', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'PPH Token v1.0', 'has the correct standard');
    });
  })
